...
const modelosGemini: Modelo[] = [
  {
    id: "gemini-1.5-flash", // Ajustado para coincidir con modelos comunes
...
export default function Inicio() {
  ...
  const [mostrarFormEspecificaciones, setMostrarFormEspecificaciones] = useState<boolean>(false);
  ...
  const enviarReceta = async () => {
    // Validación básica
    if (!solicitudReceta?.comida || solicitudReceta.comida.trim() === "") return;

    setCargando(true);
    setRespuestaIA(""); // Limpiar respuesta anterior

    try {
      // Preparamos los datos uniendo la solicitud y las especificaciones
      const payload = {
        ...solicitudReceta,
        especificaciones: especificaciones // Enviamos también las restricciones/dietas
      };
      ...
      setRespuestaIA("Error conectando con el servidor Python. Asegúrate de que 'python run.py' esté ejecutándose.");
    } finally {
      setCargando(false);
    }
  };
  ...
  const getVelocidadConfig = (velocidad: string) => {
    switch (velocidad) {
      case "ultrarrápido": return { color: "text-green-600", emoji: "⚡⚡", bg: "bg-green-50" };
      case "rápido": return { color: "text-emerald-600", emoji: "⚡", bg: "bg-emerald-50" };
      case "equilibrado": return { color: "text-blue-600", emoji: "⚖️", bg: "bg-blue-50" };
      case "potente": return { color: "text-purple-600", emoji: "🔥", bg: "bg-purple-50" };
      default: return { color: "text-gray-600", emoji: "⭐", bg: "bg-gray-50" };
    }
  };
  ...
  return (
    <main className="flex flex-col items-center !w-full gap-6 pb-20">
      ...
      <div className="flex flex-row-reverse items-center gap-1 mt-10">
        ...
        <svg viewBox="0 0 24 24" fill="none" width={70} xmlns="http://www.w3.org/2000/svg">
            <path d="M7 5C4.23858 5 2 7.23858 2 10C2 12.0503 3.2341 13.8124 5 14.584V17.25H19L19 14.584C20.7659 13.8124 22 12.0503 22 10C22 7.23858 19.7614 5 17 5C16.7495 5 16.5033 5.01842 16.2626 5.05399C15.6604 3.27806 13.9794 2 12 2C10.0206 2 8.33961 3.27806 7.73736 5.05399C7.49673 5.01842 7.25052 5 7 5Z" className="fill-orange-500"></path>
            <path d="M18.9983 18.75H5.00169C5.01188 20.1469 5.08343 20.9119 5.58579 21.4142C6.17157 22 7.11438 22 9 22H15C16.8856 22 17.8284 22 18.4142 21.4142C18.9166 20.9119 18.9881 20.1469 18.9983 18.75Z" className="fill-orange-500"></path>
        </svg>
      </div>
      ...
      <div className="bg-[#FDFBF5] text-[#343A40] p-3 border-2 border-[#8D6E63]/30 rounded-2xl flex flex-row items-center w-[700px] gap-3 shadow-lg focus-within:border-[#E67E22] focus-within:ring-2 focus-within:ring-[#E67E22]/30 transition-all duration-300 min-h-[65px] z-20">
        ...
        <div className="relative" ref={dropdownRef}>
          ...
          >
            {modeloSeleccionado.recomendado && (
              <div className="absolute -top-1 -right-1 bg-yellow-400 text-[white] text-[9px] font-bold px-1.5 py-0.5 rounded-full shadow-sm">Recomendado</div>
            )}
            <div className="p-1.5 bg-white/20 rounded-lg backdrop-blur-sm flex-shrink-0" style={{ color: modeloSeleccionado.color }}>
              {modeloSeleccionado.icono}
            </div>
            <div className="flex-1 text-left min-w-0">
              <div className="text-xs font-bold leading-tight truncate">{modeloSeleccionado.nombre}</div>
              <div className="text-[10px] opacity-90 leading-tight truncate">{modeloSeleccionado.version}</div>
            </div>
            <FaChevronDown className={`text-xs transition-transform duration-300 flex-shrink-0 ${isOpen ? "rotate-180" : ""}`} />
          </button>

          {isOpen && (
            <div className="absolute top-full left-0 mt-2 w-[420px] max-h-[400px] overflow-y-auto bg-white rounded-2xl shadow-2xl border-2 border-[#E67E22]/20 z-50 animate-fadeIn">
                ...
                {modelosGemini.map((modelo) => {
                  ...
                  return (
                    <button
                      ...
                      className={`w-full p-3.5 mb-2 rounded-xl flex items-start gap-3 transition-all duration-300 hover:scale-[1.01] hover:shadow-md border-2 relative ${modeloSeleccionado.id === modelo.id ? "bg-gradient-to-br from-[#E67E22]/10 via-[#D35400]/5 to-transparent border-[#E67E22] shadow-sm" : "bg-white border-gray-200 hover:border-[#E67E22]/40"}`}
                    >
                      <div className="p-2.5 rounded-lg flex-shrink-0 shadow-sm" style={{ backgroundColor: `${modelo.color}15`, color: modelo.color }}>{modelo.icono}</div>
                      <div className="flex-1 text-left min-w-0">
                        <div className="flex items-center justify-between mb-1">
                            <span className="font-bold text-[#343A40] text-sm block truncate">{modelo.nombre}</span>
                            {modeloSeleccionado.id === modelo.id && <FaCheck className="text-[#E67E22] text-sm animate-scaleIn flex-shrink-0 ml-2" />}
                        </div>
                        <div className="text-xs text-[#343A40]/75 mb-2 leading-relaxed">{modelo.descripcion}</div>
                      </div>
                    </button>
                  );
                })}
        ...
        <input
          ...
          onChange={(e) => updateSolicitudRecetaCallback("comida", e.target.value)}
        ...
        {imagenPreview ? (
          ...
          />
          <button onClick={(e) => { e.stopPropagation(); setImagenPreview(undefined); updateSolicitudReceta("imagen", ""); }} className="absolute -top-2 -right-2 bg-red-500 text-white rounded-full p-1 w-5 h-5 flex items-center justify-center text-xs">✕</button>
          </div>
      ...
      {respuestaIA && (
         <div className="w-[700px] bg-white p-8 rounded-3xl shadow-xl border border-[#E67E22]/20 animate-fadeIn mb-10">
            <div className="flex items-center gap-3 mb-6 border-b border-gray-100 pb-4">
               ...
               <h2 className="text-2xl font-bold text-gray-800">Receta Generada</h2>
            </div>
